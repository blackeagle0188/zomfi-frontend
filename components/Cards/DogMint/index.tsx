...
import { PET_WHITE_LIST as WHITE_LIST } from '@/constants/WHITE_LIST'
...
export default function DogMintCard(props: Props) {
  const { t } = useTranslation("");

  const { wallet, setLoading } = useAuth()
  const { type, prices, minted, setCount, total, contract } = props
    ...
    if (wallet === null) {
      ...
      if (mintRound === 1 && !tree.verify(proof, keccak256(wallet), root)) {
        ...
        if (priceId === 0) {
          ...
          if (balance >= amount) {
            try {
              tx = await usdtContract.approve(CHAIN_CONFIG.petContract.contractAddr, amount)
              await tx.wait()
            } catch (err: any) {
          ...
          if (balance >= amount) {
            try {
              tx = await busdContract.approve(CHAIN_CONFIG.petContract.contractAddr, amount)
              await tx.wait()
            } catch (err: any) {
              toast.error(err.reason)
            }
          ...
          if (balance >= amount)
            try {
              tx = await zomfiContract.approve(
                CHAIN_CONFIG.petContract.contractAddr,
                amount
              )
              await tx.wait()
            } catch (err: any) {
    ...
    <div className={`${styles.mintContainer}`}>
      {prices.length && (
        <div className={`${styles.mintWrapper} p-4 h-full`}>
          <div className={`${styles.mintAsset} ${styles.type} relative`}>
            <div
              className={`${typeClasses[type]} absolute w-full h-full top-0`}
            ></div>
            <Image
              src={`/images/dogs/${getTypeName(type)}.png`}
              className={`${styles.assetImage} absolute z-1 bottom-6`}
              alt='ethereum'
              width={'0'}
              height={'0'}
              sizes='100%'
          ...
          <div className={`${styles.assetType} flex flex-row items-center justify-between mt-4`}>
              ...
              {type !== 3 && (
                <div className='flex items-center justify-end'>
                  <div className='relative mr-3'>
                    ...
                    >
                      ...
                      <div className='ml-2 flex flex-row items-center'>
                        {showPriceDropdown ? (
                          <Icon icon="material-symbols:keyboard-arrow-up" color="white" width="24" />
                        ) : (
                          <Icon icon="material-symbols:keyboard-arrow-down" color="white" width="24" />
                        )}
                      </div>
                    </div>
                    {showPriceDropdown && (
                  ...
                  <h6 className='text-xl text-white capitalize font-bold mb-1'>
                    {displayPrice(prices[priceId].price)} <span className='text-sm'>{prices[priceId].currency}</span>
                  </h6>
                </div>
              )}
            </div>
          ...
          <div className={`mt-6`}>
            <h6 className={`${do_hyeon.className} text-white text-base`}>
              ...
              <span className='capitalize'>{typeName[props.type]}</span> 
              {t("play.Dogs Minted")}
            </h6>
          </div>
          {type !== 3 ? (
            ...
            <div className='mt-4 flex flex-row'>
              <h6 className={`${do_hyeon.className} text-white`}>{t("play.Mint Description")}</h6>
            </div>
          )}
        </div>
      )}
